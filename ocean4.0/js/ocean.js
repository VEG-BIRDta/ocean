...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function refreshFishList() {
    ...
    fishes.forEach(fish => {
      ...
      item.innerHTML = `
        <div class="info">
          <div class="name">${fish.name} ${typeBadge}</div>
          <div class="story">${fish.story || '无介绍'}</div>
        </div>
        ${!isStoreCharacter ? `<button class="delete-btn" data-id="${fish.id}">删除</button>` : ''}
      `;
    ...
    decors.forEach(decor => {
      ...
      item.innerHTML = `
        <div class="info">
          <div class="name">${decor.name} <span class="decor-badge">装饰</span></div>
          <div class="story">${decor.story || '无介绍'}</div>
        </div>
        <button class="delete-btn" data-id="${decor.id}" data-type="decor">删除</button>
      `;
      fishItemsContainer.appendChild(item);
    });
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
      btn.addEventListener('click', function(e) {
        ...
        if (type === 'decor') {
          // 删除装饰物品
          if (confirm(`确定要删除「${decors.find(d => d.id === id)?.name || '这个装饰品'}」吗？`)) {
            deleteDecorById(id);
          }
        } else {
          // 删除小鱼
  ...
  function deleteDecorById(id) {
    const index = decors.findIndex(decor => decor.id === id);
    if (index === -1) return;
    
    if (currentDecor && currentDecor.id === id) {
      if (currentDecorBubble) document.body.removeChild(currentDecorBubble);
      currentDecorBubble = null;
      currentDecor = null;
    }
